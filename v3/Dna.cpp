...
Dna::Dna(int length, Threefry::Gen &&rng) : length_(length) {
    seq_ = new char[length*2];
    // Generate a random genome
    for (int32_t i = 0; i < length; i++) {
        seq_[i] = '0' + rng.random(NB_BASE);
    }
    memcpy(seq_+length, seq_, length * sizeof(char));
}

Dna::Dna(const Dna &clone) {
    length_ = clone.length_;
    
    seq_ = new char[length_*2];
    memcpy(seq_, clone.seq_, length_ * 2 * sizeof(char));
    
}

void Dna::save(gzFile backup_file) {
    gzwrite(backup_file, &length_, sizeof(length_));
    gzwrite(backup_file, seq_, length_ * 2 * sizeof(seq_[0]));
}
...
void Dna::load(gzFile backup_file) {
    ...
    char tmp_seq[dna_length * 2];
    gzread(backup_file, tmp_seq, dna_length * sizeof(tmp_seq[0]));
    seq_ = tmp_seq;
    memcpy(seq_+length_, seq_, length_ * sizeof(char));
}
...
void Dna::remove(int pos_1, int pos_2) {
    ...
    char* new_seq = new char[new_length*2];
...
void Dna::do_switch(int pos) {
    ...
    if (seq_[pos+length_] == '0') seq_[pos+length_] = '1';
    else seq_[pos+length_] = '0';
}
...
void Dna::do_duplication(int pos_1, int pos_2, int pos_3) {
    ...
    if (pos_1 < pos_2) {
        ...
        memcpy(seq_+pos_1, seq_dupl, length_dupl * sizeof(char));
